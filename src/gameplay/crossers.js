...
import { buildPerson } from '../world/props/npcs.js';
...
// Shared crossing driver. The family walks single-file from one verge (+x) to
// the other (-x); `done` flips once they've cleared the carriageway so the ride
// flows on, while they keep ambling off. `bob` animates each member's gait.
// `face` orients them along the direction of travel (-x): models built facing
// +x use Math.PI (default); a person built facing +z uses -Math.PI/2. Getting
// this wrong makes them walk sideways — the "crab walk".
function crosser(g, members, { speed = 1.35, spacing = 0.5, face = Math.PI, bob } = {}) {
  let crossing = false;
  let progress = -6.5;
  return {
    group: g,
...
export function buildCow() {
  ...
  return crosser(g, [cow], {
    ...
    bob: (m, i, t) => {
      ...
      // legs swing FORE-AFT (about z, the body's long axis) in a diagonal gait —
      // rotation.x here made them kick sideways = the crab walk
      const gaitPh = [0, Math.PI, Math.PI, 0];
      for (let k = 0; k < legs.length; k++) legs[k].rotation.z = Math.sin(t * 3.2 + gaitPh[k]) * 0.32;
      tail.rotation.z = 0.35 + Math.sin(t * 1.5) * 0.25;
    },
  });
}

// two elderly folk shuffling across, one with a walking stick. Uses the
// articulated pedestrian model (proper torso, jointed arms/legs, feet & hands)
// with grey hair + a stoop, so they read as real people, not blocks.
export function buildElderCrossing() {
  const g = new THREE.Group();
  const members = [];
  for (let i = 0; i < 2; i++) {
    const person = buildPerson(4207 + i * 31, { elderly: true });
    person.group.userData.walk = person.walk;
    if (i === 0) {
      const stick = new THREE.Mesh(
        new THREE.CylinderGeometry(0.013, 0.017, 0.66, 6),
        new THREE.MeshStandardMaterial({ color: 0x8a5a34, roughness: 0.6 }),
      );
      stick.position.set(0.24, 0.33, 0.2); // held out front, planted on the road
      person.group.add(stick);
    }
    g.add(person.group);
    members.push(person.group);
  }
  // person model faces +z, so face -PI/2 to walk along -x; slow shuffle
  return crosser(g, members, {
    speed: 1.05, spacing: 0.85, face: -Math.PI / 2,
    bob: (m, i, t) => { m.userData.walk?.(t + i * 1.9, 0.7); },
  });
}
